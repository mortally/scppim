...
public class OSSCDP_BidEvaluatorSMU_DR extends
		SelfConfirmingDistributionPricePrediction {
	
	private static final long serialVersionUID = 100L;
	private static final double KAPPA = 1.25;
	private double[] demand;
	private double avgDemand;
	private double[] adjustedDemand;
	
	public OSSCDP_BidEvaluatorSMU_DR(int index)
	{
		super(index);
	}
	...
	public double[] bid(InformationState s)
	{
		...
		adjustedDemand = new double[NUM_GOODS];
		avgDemand = 0.0;
		//int noPredCount = 0;
		...
		else
		{
			...
			for (int i=0;i<NUM_GOODS;i++)
			{
				avgDemand += (demand[i] / (double)NUM_GOODS);
			}
			
			for (int i=0;i<NUM_GOODS;i++)
			{
				adjustedDemand[i] = (demand[i] + ((demand[i] - avgDemand)*KAPPA)) / avgDemand;
			}
	...
	private double[] sampleMV()
	{
		...
		for (int e=0;e<NUM_SCENARIO;e++)
		{
			...
			for (int i=0;i<NUM_GOODS;i++)
			{
				...
				if (pos >= 0) 
				{
					...
					scenarios[e][i] = pos;// + (KAPPA * NUM_AGENT * HIERARCHICAL_REDUCTION_LEVEL);
				}
				else
				{
					scenarios[e][i] = ((pos * -1) - 1);// + (KAPPA * NUM_AGENT * HIERARCHICAL_REDUCTION_LEVEL);
				}
			}
		}
		...
		for (int k=0;k<NUM_CANDIDATE_BID;k++)
		{
			...
			for (int i=0;i<NUM_GOODS;i++)
			{
				candidateBid[i] = candidateBid[i] * adjustedDemand[i];
			}
			if (PRINT_DEBUG)
			{
				System.out.println("Agent " + this.getIndex() + "'s adjusted demand");
				for (int i=0;i<NUM_GOODS;i++)
				{
					System.out.print(adjustedDemand[i] + " ");
				}
				System.out.println();
